# SnapStrip Studio 📸

## 🎉 [**TRY IT LIVE**](https://thanhnguyxn.github.io/Photo-Booth/)

A free online photo booth with 12 creative templates. Create stunning photo strips in seconds!

---

## ✨ Features

- 🎭 **12 Creative Templates** - Newspaper, Magazine, Comic, Polaroid, Passport, Movie, Vintage, Neon, Scrapbook, Minimalist, Instagram, Classic
- 📷 **Camera or Upload** - Take photos with webcam or upload existing ones
- 🎨 **6 Photo Filters** - None, B&W, Sepia, Vintage, Cool, Warm
- 📥 **Download & Share** - Export as JPG or share to social media
- 📱 **Fully Responsive** - Works on desktop, tablet, and mobile
- 🔒 **Privacy First** - All processing in browser, no uploads
- 🆓 **100% Free** - No watermarks, no limits

---

## 🎭 Templates

| Template | Style |
|----------|-------|
| 📰 Newspaper | Breaking news |
| 📸 Magazine | Fashion & elegant |
| 💥 Comic Book | Superhero vibes |
| 📷 Polaroid | Vintage instant |
| 🛂 Passport | Professional ID |
| 🎬 Movie Poster | Cinematic |
| ✨ Vintage | Retro & classic |
| ⚡ Neon | Electric glow |
| 📌 Scrapbook | Crafty & colorful |
| ◻️ Minimalist | Clean & simple |
| 📱 Instagram | Social ready |
| Classic | Traditional booth |

---

## 💡 Use Cases

Perfect for:
- 🎉 Parties & Events
- 💑 Weddings
- 🎓 Graduations
- 🎂 Birthdays
- 👨‍👩‍👧‍👦 Family Reunions
- 📸 Fun with Friends
- 🏢 Corporate Events

---

## 🎯 Tech Stack

- HTML5 + Canvas API
- Tailwind CSS
- Vanilla JavaScript
- WebRTC API
- Web Share API

---

## 🚀 Quick Start

Just visit: **[https://thanhnguyxn.github.io/Photo-Booth/](https://thanhnguyxn.github.io/Photo-Booth/)**

1. Click **START**
2. Choose **Layout** (2-6 poses)
3. Select **Template** (12 options)
4. **Capture** or **Upload** photos
5. **Download** your photo strip!

---

## 📱 Browser Support

✅ Chrome 53+ • Firefox 36+ • Safari 11+ • Edge 12+

---

## 👨‍💻 Author

**Thanh Nguyen**  
📧 thanhnguyentuan2007@gmail.com  
🐙 [@ThanhNguyxn](https://github.com/ThanhNguyxn)

---

## 📄 License

© 2025 SnapStrip Studio. All Rights Reserved.

---

<div align="center">

### 👉 [**START CREATING**](https://thanhnguyxn.github.io/Photo-Booth/) 👈

⭐ Star this repo if you like it!

Made with 📸 and ❤️

</div>
